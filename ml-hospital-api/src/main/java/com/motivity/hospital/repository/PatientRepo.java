...
public interface PatientRepo extends CrudRepository<Patients, Integer>{
	...
	public Patients loginPatient(String email,String password);
	 ...
	 @Transactional
	    @Modifying
	    @Query("update Patients p set p.patientName=:patientName,p.email=:email, p.phoneNo=:phoneNo,p.Patient_age=:Patient_age,p.patientAddress=:patientAddress where p.patientId=:patientId")
	    public int updatePatient(String patientName,String email,String phoneNo,String Patient_age,String patientAddress,int patientId);
	 @Transactional
	    @Modifying
